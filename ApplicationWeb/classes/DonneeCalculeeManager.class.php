...
class DonneeCalculeeManager {
    ...
    {
        $req = $this->db->prepare(
            "SELECT idType, nomFormule, tableauIdParams FROM donnee_calculee WHERE idType = :idDonneeCalculee"
        );
    ...
    function recupererDonneeCalculeeViaListeIdDonneeCalculee($listeIdDonneeCalculee)
    {
        $listeDonneeCalculee = array();
        foreach ($listeIdDonneeCalculee as $donneeCalcule) {
          $listeDonneeCalculee[] = $this->recupererDonneeCalculeeViaIdDonneeCalculee($donneeCalcule);
        }

        return $listeDonneeCalculee;
    }
}
