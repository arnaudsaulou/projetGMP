...
class AttribueManager
{
     ...
     */
    public function countNombreDeFoisQuunSujetAEteAttribueAUnEtudiant($idEtudiant, $idEnonce)
    {
     ...
     * @return bool
     */
    public function supprimerAttribueViaIdSujet($idSujet) {
        $req = $this->db->prepare("DELETE FROM attribue WHERE idSujet = :idSujet");
        $req->bindValue(':idSujet', $idSujet, PDO::PARAM_STR);
        $result = $req->execute();
        $req->closeCursor();
        return $result;
